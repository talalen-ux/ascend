/**
 * Post-deploy: seed a tiny Uniswap V2 ascend/WETH pool and lock the LP.
 *
 * Why: trackers like Dexscreener and GeckoTerminal index Uniswap pools.
 * Without one, ascend has no public price feed even though the engine is
 ...
 *   1. quotes & buys SEED_ETH worth of ascend from the engine
 *   2. approves the Uniswap V2 router for the resulting ascend
 ...
 *   ASCEND_ENGINE      0x... engine address (from deploy output)
 *   UNI_V2_ROUTER    0x... uniswap v2 router (mainnet: 0x7a25...488D)
 *   SEED_ETH         decimal ETH amount for the seed (default: 0.1)
 *
 * Run:
 *   PRIVATE_KEY=0x... RPC_URL=... ASCEND_ENGINE=0x... UNI_V2_ROUTER=0x... \
...
async function main() {
  ...
  const engineAddr = required("ASCEND_ENGINE") as `0x${string}`;
  ...
  console.log(`seed amount:  ${seedEth} ETH (× 2 — half buys ascend, half pairs as ETH)\n`);

  // ----------------------------------------------------------------- 1. buy ascend from engine
  const seedWei = parseEther(seedEth);
  const [quotedAscend] = await pub.readContract({
    address: engineAddr,
    abi: ENGINE_ABI,
    functionName: "quoteBuy",
    args: [seedWei],
  });
  console.log(`quoting:  ${seedEth} ETH → ${formatEther(quotedAscend)} ascend`);
  ...
  const ascendAddr = await pub.readContract({
    address: engineAddr,
    abi: ENGINE_ABI,
    functionName: "ascend",
  });
  ...
  console.log(`got:      ${formatEther(balance)} ascend\n`);

  // ----------------------------------------------------------------- 2. approve router
  console.log(`approving router for ascend…`);
  ...
  console.log(`adding liquidity: ${formatEther(balance)} ascend + ${formatEther(ethForLp)} ETH`);
  ...
  const addHash = await wallet.writeContract({
    ...
    args: [
      ascendAddr,
      balance,
      (balance * 99n) / 100n, // 1% slippage on ascend side
      (ethForLp * 99n) / 100n,
      account.address,
      deadline,
    ],
